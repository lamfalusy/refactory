...
$(function() {
	...
	emailRegex = /^[a-zA-Z0-9.!#$%&'*+\/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/, fullName = $("#fullName"), name = $("#name"), email = $("#email"), password = $("#password"), role = $("#role"), allFields = $(
			[]).add(fullName).add(name).add(email).add(password).add(role), tips = $(".validateTips");
	...
	function addUser() {
		...
		valid = valid
				&& checkRegexp(fullName, /^[a-z]([a-z\s])+$/i,
						fullNameRegexError);
		valid = valid
				&& checkRegexp(name, /^[a-z]([0-9a-z_\s])+$/i,
						loginNameRegexError);
		...
		if (valid) {
			...
			$("#users tbody").append(
					"<tr>" + "<td>" + fullName.val() + "</td>" + "<td>"
							+ name.val() + "</td>" + "<td>" + email.val()
